...
function smwfuzzy.onTickEndNPC(v)
    ...
    if v.data.startX == nil then
        v.data.startX = v:mem(0xA8, FIELD_DFLOAT)
    ...
    for k,p in ipairs(Player.get()) do
        if p.standingNPC and p.standingNPC == v then
			...
			if p.forcedState == 0 then
				v.speedX = v.data.startDirection * 2
			else
				v.speedX = 0
			end
    ...
    if v.direction == -v.data.startDirection and (
        (v.data.startDirection == 1 and v.data.lastX > v.data.startX and v.x <= v.data.startX) or
        (v.data.startDirection == -1 and v.data.lastX < v.data.startX and v.x >= v.data.startX)) then
            v.data.active = nil
            v.direction = v.data.startDirection
            v.speedX = 0
            v.x = v.data.startX
